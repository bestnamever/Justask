...
class My_fav_answer_dia(context: Context) : Dialog(context) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var docRef = db.collection("users").document(currentUserEmail).collection("fav_user").get()
        docRef.addOnSuccessListener { documents ->
            for (document in documents) {
                if (document != null) {
                    ...
                    list.add(
                        Item(
                            document.getString("Fav_user").toString(),
                            document.getString("Fav_answer").toString()
                        )
                    )
